...
### Fixed
- **Tab Track Width & Transform**: Accurate sliding for any number of tabs
  - Switched to pixel-based pane sizing and track transform tied to wrapper width
  - Removed hard-coded `width: 200%` and `flex: 0 0 100%` rules in styles that caused mismatch
  - Added ResizeObserver and window resize handling to re-measure and keep panes aligned

### Docs
- Added `docs/lite-tab-group.md` with API, usage, and examples
- Updated READMEs to list LiteTabGroup and link docs

## [1.4.3] - 2025-10-13

### Added
- **LiteTable Row Selection**: Checkbox column for multi-row selection
  - New `type: 'select'` column type for rendering selection checkboxes
  ...
  - `selectionChange` output event emits array of selected rows on any selection change
  - Selection state tracked internally with `Set<T>` for efficient lookups
...
- **LiteTable API**: New selection helper methods
  ...
  - `toggleRow(row, checked)` - Toggle individual row selection
